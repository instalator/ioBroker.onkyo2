...
function iscp_to_command(iscp_message){
    ...
    let command, value, result = [];

    if(command === 'NLT'){
        console.log();
    }

    if (iscp_message.includes('ISCP') && !iscp_message.includes('NLS')){
        let arr = iscp_message.split('ISCP');
        arr.forEach((val, index)=>{
            result.push({});
            console.log('val = ' + JSON.stringify(val));
            if (!/[A-Z]/.test(val[0]) && !/[A-Z]/.test(val[1]) && !/[A-Z]/.test(val[2])) {
                val = val.slice(14);
            }
            console.log('>>>> ' + val);
            val = val.replace(/\n|\r/g, '');
            ...
            Object.keys(COMMANDS).forEach((zone) => {
                if (typeof COMMANDS[zone][command] !== 'undefined'){
                    ...
                    if (typeof zone_cmd.values[value] !== 'undefined' && in_modelsets(VALUE_MAPPINGS[zone][command][zone_cmd.values[value].name].models)){
                        ...
                        result[index].argument = parseInt(value, 16);
                    } else {
                        Object.keys(VALUE_MAPPINGS[zone][command]).forEach((key) => {
                            if (~config.modelsets.indexOf(VALUE_MAPPINGS[zone][command][key].models)){
        ...
        result[0] = {};
        result[0].iscp_command = command;
        //console.log('value = ' + JSON.stringify(value));
        Object.keys(COMMANDS).forEach((zone) => {
            if (typeof COMMANDS[zone][command] !== 'undefined'){
                const zone_cmd = COMMANDS[zone][command];
                result[0].command = zone_cmd.name;
                result[0].zone = zone;
                if (typeof zone_cmd.values[value] !== 'undefined' && in_modelsets(VALUE_MAPPINGS[zone][command][zone_cmd.values[value].name].models)){
                    result[0].argument = zone_cmd.values[value].name;
                } else if (typeof VALUE_MAPPINGS[zone][command].INTRANGES !== 'undefined' && /^[-+0-9a-fA-F]+$/.test(value)){
                    // It's a range so we need to convert args from hex to decimal
                    result[0].argument = parseInt(value, 16);
                } else {
                    Object.keys(VALUE_MAPPINGS[zone][command]).forEach((key) => {
                        if (~config.modelsets.indexOf(VALUE_MAPPINGS[zone][command][key].models)){
                            result[0].argument = value;
                        }
                    });
                }
            }
...
self.connect = function (options){
    ...
    */
    ...
    eiscp.on('connect', () => {
        ...
        result.forEach((res)=>{
            //res.iscp_command = res.iscp_command;
...
self.get_command = function (zone, command, callback){
    ...
    async.each(Object.keys(VALUE_MAPPINGS[zone][cmd]), (val, cb) => {
        if (val !== 'query' && in_modelsets(VALUE_MAPPINGS[zone][cmd][val].models)){
            ...
            values[val] = {desc: descVal, type: 'string'};
